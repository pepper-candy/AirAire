...
export function EquityChart({ points }: Props) {
  ...
  if (!geom) {
    return <div className="equity-empty">Waiting for two session snapshots</div>;
  }
  ...
  return (
    ...
    >
      <svg className="equity-svg" viewBox="0 0 100 42" preserveAspectRatio="none" aria-hidden>
        <path d={geom.d} fill="none" stroke="#8fbf9f" strokeWidth="1.1" />
        {active ? (
          <>
            <line x1={active.x} x2={active.x} y1="4" y2="40" stroke="#9aa3b2" strokeWidth="0.35" />
            <circle cx={active.x} cy={active.y} r="1.3" fill="#8fbf9f" />
          </>
        ) : null}
      </svg>
      <div className="equity-readout">
        {active ? (
          <>
            ...
            <strong>{money(active.p.equity)}</strong>
          </>
        ) : (
