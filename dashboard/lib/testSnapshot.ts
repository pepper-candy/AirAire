...
function makeTestEquity(): { t: string; equity: number }[] {
  ...
  for (let i = 5 * 24 * 6; i >= 0; i -= 1) {
    ...
    const hkHour = Number(parts.find((p) => p.type === "hour")?.value);
    if (weekday === "Sat" || weekday === "Sun" || !Number.isFinite(hkHour)) {
      continue;
    }
    if (hkHour < 9 || hkHour >= 16) {
      continue;
    }
    equity += Math.sin(i / 7) * 180 + (i % 11) * 8 - 40;
...
export function makeTestSnapshot(): SnapshotResponse {
  ...
  const equitySeries = makeTestEquity();
  return {
    latest,
    equitySeries,
    equityMeta: {
      ...
      rawCount: equitySeries.length,
      shownCount: equitySeries.length,
      bucketMinutes: 10,
    },
    stale: false,
