...
def _slice_panel(panel: pd.DataFrame) -> pd.DataFrame:
    """Train windows only. Holdout still uses ``_FULL_PANEL`` (full parquet)."""
    ...
    logger.info(
        ...
        len(out),
        len(panel),
        int(dt.loc[mask].dt.normalize().nunique()) if mask.any() else 0,
    )
    if out.empty:
        raise ValueError("Train slice is empty. Check --start / --end against enhanced_v3.parquet.")
    return out
...
def _sanitize_and_slice(panel: pd.DataFrame) -> pd.DataFrame:
    """Keep the full tape for holdout; hand the trainer only the slice."""
    orig = _ORIG_SANITIZE
    cleaned = orig(panel) if orig is not None else panel
    _remember_panel(cleaned)
...
def train(
    *,
    ...
    global _SLICE_START, _SLICE_END
