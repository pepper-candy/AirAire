...
V3 / V3.1: weights in [-1, +1]. HK SIMULATE is still reduce-only; US SIMULATE
can short without a funded US stock account.
...
def catch_up_env_v3(
    ...
    *,
    ...
    if any(float(v or 0.0) > 0 for v in live_px.values()):
        _mark_books(state, live_px)
    else:
        state.equity = float(env._last_equity)

    state.last_bar_datetime = str(caught_dt)
    logger.info(
        "V3 catch-up complete (no orders). env_before=%s env_now=%s bar=%d/%d rebuilt=%s "
        "cash=%.2f equity=%.2f seek=%s clock=%s",
        before_dt,
        caught_dt,
        env._bar_index,
...
def _mtm(holdings: dict[str, float], prices: dict[str, float], prefix: str) -> float:
    return sum(
...
def _hk_equity(state, prices: dict[str, float]) -> float:
    return float(state.cash) + _mtm(state.holdings, prices, "HK.")


def _us_equity(state, prices: dict[str, float]) -> float:
    return float(getattr(state, "us_cash", US_INITIAL_CASH)) + _mtm(state.holdings, prices, "US.")


def _mark_books(state, prices: dict[str, float]) -> None:
    if not any(float(v or 0.0) > 0 for v in (prices or {}).values()):
        return
    state.equity = _hk_equity(state, prices)


def _apply_cash(state, ticker: str, flow: float) -> None:
    """HK and US SIMULATE are two accounts. Never debit HK HKD for a US fill."""
...
def _clamp_delta(ticker: str, delta: int, current: float, *, reduce_only: bool) -> int:
    # Futu US SIMULATE accepts shorts even without a funded US stock account. HK does not.
    if str(ticker).startswith("US."):
        return delta
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument(
        "--allow-shorts",
        action="store_true",
        help="Let HK go short too. US SIMULATE already allows shorts without a funded US stock account.",
    )
    p.add_argument(
        "--push-dashboard",
        action="store_true",
