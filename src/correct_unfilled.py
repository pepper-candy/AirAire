...
def patch_state_v3(payload: dict[str, Any]) -> bool:
    ...
    if "us_cash" in payload:
        state.us_cash = float(payload["us_cash"])
...
def rebuild_from_fills(payload: dict[str, Any]) -> dict[str, Any]:
    """HK/US books from qty×price. Phantom CATL sells are CANCEL. No OpenD fees."""
    from src.dashboard_push import load_recent_fills
    from src.utils import US_INITIAL_CASH

    phantom = {str(row["order_id"]) for row in UNFILLED_SELLS}
    rows = load_recent_fills(10_000) or list(payload.get("fills") or [])
    fills: list[dict[str, Any]] = []
    holdings = {t: 0.0 for t in CORE_TICKERS}
    hk_cash = float(payload.get("initial_cash") or INITIAL_CASH)
    us_cash = float(US_INITIAL_CASH)
    marks: dict[str, float] = {}
    chrono = sorted(rows, key=lambda row: str(row.get("time") or ""))
    for raw in chrono:
        item = dict(raw)
        oid = str(item.get("order_id") or "")
        if oid in phantom:
            item["side"] = "CANCEL"
            item["reason"] = "CANCEL unfilled limit — shares restored. " + str(item.get("reason") or "")
        fills.append(item)
        ...
        if ticker not in holdings or qty <= 0 or px <= 0:
            continue
        holdings[ticker] += qty if side == "BUY" else -qty
        flow = -qty * px if side == "BUY" else qty * px
        if ticker.startswith("US."):
            us_cash += flow
        else:
            hk_cash += flow

    if not any(abs(v) > 1e-9 for v in holdings.values()):
        for ticker in CORE_TICKERS:
            holdings[ticker] = float((payload.get("holdings") or {}).get(ticker, 0.0) or 0.0)

    hk_mtm = sum(holdings[t] * float(marks.get(t) or 0.0) for t in CORE_TICKERS if t.startswith("HK."))
    us_mtm = sum(holdings[t] * float(marks.get(t) or 0.0) for t in CORE_TICKERS if t.startswith("US."))
    out = json.loads(json.dumps(payload))
    out["cash"] = float(hk_cash)
    out["us_cash"] = float(us_cash)
    out["equity"] = float(hk_cash + hk_mtm)
    out["us_equity"] = float(us_cash + us_mtm)
    out["holdings"] = holdings
    out["prices"] = marks
    out["pnl"] = float(out["equity"]) - float(out.get("initial_cash") or INITIAL_CASH)
    out["fills"] = fills[-500:]
    out["kind"] = "live"
    out["updated_at"] = hk_now_iso()
    out["last_reason"] = (
        "Fill-log rebuild (qty×price, no Futu fees). "
        f"CATL 8899494/8899530 CANCEL. HK cash={hk_cash:.2f} CATL={holdings['HK.03750']:.0f}."
    )
    logger.info(
        "Fill-log book HK cash=%.2f equity=%.2f CATL=%.0f | US cash=%.2f equity=%.2f",
        hk_cash,
        out["equity"],
        holdings["HK.03750"],
        us_cash,
...
def run(*, dry_run: bool = False, from_fills: bool = True) -> int:
    ...
    if from_fills:
        corrected = rebuild_from_fills(payload)
    else:
        if catl >= 1299:
            logger.info("CATL already restored (%.0f). Not reversing twice. Use --from-fills to rebuild cash.", catl)
            return 0
        corrected = reverse_unfilled(payload)
    if dry_run:
        logger.info(
            "Dry-run cash=%.2f equity=%.2f CATL=%.0f us_cash=%.2f",
            corrected["cash"],
            corrected["equity"],
            corrected["holdings"]["HK.03750"],
            float(corrected.get("us_cash") or 0.0),
        )
        return 0
    if not configured():
    ...
    logger.info("Blotter updated from fill log. Stop/restart V3 so the pickle stays.")
    return 0
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Fix CATL cancels from the fill log (not Futu fees).")
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument(
        "--reverse",
        action="store_true",
        help="Old one-shot: add 1100 CATL and subtract proceeds. Skip if CATL already restored.",
    )
    args = parser.parse_args()
    raise SystemExit(run(dry_run=args.dry_run, from_fills=not args.reverse))
